...
export function Header() {
  ...
  return (
    <header
      className={`${scrolled ? 'bg-white/30 backdrop-blur-md backdrop-saturate-120 shadow-xl' : 'bg-transparent backdrop-blur-lg'} hidden lg:block fixed top-[70px] md:top-[70px] z-40 left-0 right-0 transition-all duration-300`}
    >
          ...
          <nav className="hidden lg:flex items-center space-x-0.5">
            {navigation.map((item, index) => (
              <div key={item.name} className="flex items-center">
                ...
                {index < navigation.length - 1 && <span className="text-white/60 mx-1">|</span>}
              </div>
            ))}
          </nav>
        </div>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild className="lg:hidden" data-header-sheet-trigger>
              <Button
                variant="ghost"
                size="icon"
                className={`${scrolled ? 'text-gray-900' : 'text-white drop-shadow-lg'}`}
              >
            ...
            <SheetContent
              side="right"
              className="w-[350px] sm:w-[350px] bg-white/95 backdrop-blur-md border-l border-gray-200/50"
            >
              <nav className="flex flex-col space-y-1 mt-2">
                ...
                <Button
                  asChild
                  className="mt-6 bg-[#1B5D93]/80 text-white hover:bg-[#1B5D93]/90 transition-all duration-300 rounded-lg backdrop-blur-sm"
                >
